...
func AllK8sSecrets(namespaceName string) utils.HttpResult {
	result := []v1.Secret{}
...
func UpdateK8sSecret(data v1.Secret) utils.HttpResult {
	kubeProvider := NewKubeProvider()
...
func DeleteK8sSecret(data v1.Secret) utils.HttpResult {
	kubeProvider := NewKubeProvider()
	secretClient := kubeProvider.ClientSet.CoreV1().Secrets(data.Namespace)
...
func DescribeK8sSecret(namespace string, name string) utils.HttpResult {
...
func CreateK8sSecret(data v1.Secret) utils.HttpResult {
	kubeProvider := NewKubeProvider()
