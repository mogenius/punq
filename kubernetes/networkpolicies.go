...
func AllNetworkPolicies(namespaceName string) utils.HttpResult {
...
func UpdateK8sNetworkPolicy(data v1.NetworkPolicy) utils.HttpResult {
...
func DeleteK8sNetworkPolicy(data v1.NetworkPolicy) utils.HttpResult {
	kubeProvider := NewKubeProvider()
	client := kubeProvider.ClientSet.NetworkingV1().NetworkPolicies(data.Namespace)
...
func DescribeK8sNetworkPolicy(namespace string, name string) utils.HttpResult {
...
func CreateK8sNetworkpolicy(data v1.NetworkPolicy) utils.HttpResult {
	kubeProvider := NewKubeProvider()
	client := kubeProvider.ClientSet.NetworkingV1().NetworkPolicies(data.Namespace)
