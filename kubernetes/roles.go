...
func AllRoles(namespaceName string) utils.HttpResult {
	result := []v1.Role{}
...
func UpdateK8sRole(data v1.Role) utils.HttpResult {
...
func DeleteK8sRole(data v1.Role) utils.HttpResult {
	kubeProvider := NewKubeProvider()
	client := kubeProvider.ClientSet.RbacV1().Roles(data.Namespace)
...
func DescribeK8sRole(namespace string, name string) utils.HttpResult {
...
func CreateK8sRole(data v1.Role) utils.HttpResult {
	kubeProvider := NewKubeProvider()
	client := kubeProvider.ClientSet.RbacV1().Roles(data.Namespace)
